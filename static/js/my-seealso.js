...
function SeeAlsoView(p) {
    ...
    this.emptyHTML = (typeof p.emptyHTML == "string" || typeof p.emptyHTML == "function")
        ? p.emptyHTML : "";
    this.maxItems = typeof p.maxItems == "number" ? p.maxItems : 40;
...
function BibSonomyTaggedService(labels) {
    ...
    this.queryURL = function(identifier, callback) {
        var url = "http://www.bibsonomy.org/json/tags/bibtex/" + identifier;
        if (callback) url += "?callback=" + callback;
        return url;
    }
    ...
    this.createResponse = function(data, identifier) {
        ...
        if (data && data.tags && data.tags.length > 0) {
                var url = "http://www.bibsonomy.org/bibtex/" + identifier;
                var descr = "";
...
function GoogleBookSearchService(labels) {
    ...
    this.queryURL = function(identifier, callback) {
        var url = "http://books.google.com/books?jscmd=viewapi";
        if (callback) url += "&callback=" + callback;
...
function MediaWikiTitleService(baseURL) {
    this.baseURL = baseURL || "http://en.wikipedia.org/";
...
mySeeAlso.services =  {
  ...
  'gnd2findbuch' : new SeeAlsoService("http://beacon.findbuch.de/seealso/pnd-aks")
};
